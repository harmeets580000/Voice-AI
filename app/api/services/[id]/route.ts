...
export const PATCH = handleRoute(async (req, ctx) => {
  ...
  const body = UpdateService.parse(await req.json());
  return ok({ service: await updateService(organizationId, id, body) });
});
...
export const DELETE = handleRoute(async (req, ctx) => {
  ...
  assertRole(principal, [Role.ORG_ADMIN, Role.SUPER_ADMIN]);
  const { id } = await (ctx as Ctx).params;
  return ok(await deleteService(organizationId, id));
});
